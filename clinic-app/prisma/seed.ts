...
async function main() {
  ...
  const doctors = [
    ...
    { code: 3, name: "RAMANA REDDY", commissionPercent: 75, tdsPercent: 10.3, designationId: 1, permissionLevel: 3, password: "doctor", isConsultant: true },
    { code: 4, name: "RAVINDER", commissionPercent: 50, tdsPercent: 10.3, designationId: 1, isConsultant: true },
    { code: 5, name: "ANITHA", commissionPercent: 70, tdsPercent: 5.1, designationId: 1, isConsultant: true },
    { code: 6, name: "BABU RAM", commissionPercent: 50, tdsPercent: 5.1, designationId: 1, isConsultant: true },
    { code: 7, name: "TAUFIQ", commissionPercent: 0, designationId: 1 },
    { code: 9, name: "SHEETAL", commissionPercent: 45, tdsPercent: 5.1, designationId: 1, isConsultant: true },
    { code: 10, name: "HEMANTH", commissionPercent: 50, tdsPercent: 5.1, designationId: 1, isConsultant: true },
    { code: 16, name: "VIVEK", commissionPercent: 60, tdsPercent: 5.1, designationId: 1, isConsultant: true },
    { code: 17, name: "RATHIKA RAI", commissionPercent: 25, tdsPercent: 5.15, designationId: 1, isConsultant: true },
    { code: 21, name: "GIRISH KADRI", commissionPercent: 45, tdsPercent: 5.1, designationId: 1, isConsultant: true },
    { code: 28, name: "GAUTAM", commissionPercent: 50, tdsPercent: 11.33, designationId: 1, isConsultant: true },
    { code: 31, name: "MADHU", commissionPercent: 50, tdsPercent: 11.33, mobile: "9849553445", designationId: 1, isConsultant: true },
    { code: 35, name: "BHADRA RAO", commissionPercent: 0, commissionRate: 750, tdsPercent: 11.33, designationId: 1, isConsultant: true },
  ...
  // ==========================================
  // SAMPLE APPOINTMENTS — realistic clinic day demo
  // ==========================================
  const tomorrow = new Date(today.getTime() + 86400000);

  const appointments = [
    // ═══ SURENDER (BDS, in-house): Mix of consultation + treatment patients ═══
    ...
    { patientId: 44, doctorId: surender!.id, date: tomorrow, timeSlot: "2:00 PM", status: "SCHEDULED", reason: "Scaling + cleaning", createdById: kazim!.id, roomId: 3 },
    { patientId: 47, doctorId: ramana!.id, date: tomorrow, timeSlot: "3:00 PM", status: "SCHEDULED", reason: "Crown prep", createdById: kazim!.id, roomId: 2 },
    ...
    { patientId: 33, doctorId: anitha!.id, date: daysAgo(1), timeSlot: "3:00 PM", status: "NO_SHOW", reason: "Ortho adjustment", createdById: kazim!.id },
    { patientId: 29, doctorId: surender!.id, date: daysAgo(1), timeSlot: "9:30 AM", status: "COMPLETED", reason: "Extraction", createdById: kazim!.id, roomId: 3 },
    { patientId: 38, doctorId: surender!.id, date: daysAgo(1), timeSlot: "11:00 AM", status: "COMPLETED", reason: "Composite filling", createdById: kazim!.id, roomId: 3 },
  ];
  ...
  // Plan-linked appointments are set up at the end, after treatment plans are created
  ...
  // Auto-suggest links: RCT → Crown PFM
  if (opRCT && opCeramicPFM) {
    await prisma.operation.update({
      where: { id: opRCT.id },
      data: { suggestsOperationId: opCeramicPFM.id },
    });
  }
  ...
  console.log(`   - ${appointments.length} appointments (with plan-linked demo)`);
  console.log("   - 5 rooms");
  ...
  const implantPlanItem1 = await prisma.treatmentPlanItem.findFirst({
    where: { plan: { patientId: 28 }, sortOrder: 1 },
  });
  const patient28Appt = await prisma.appointment.findFirst({
    where: { patientId: 28, doctorId: ramana!.id, date: { gte: today, lt: tomorrow } },
  });
  if (implantPlanItem1 && patient28Appt) {
    await prisma.appointment.update({
      where: { id: patient28Appt.id },
      data: { planItemId: implantPlanItem1.id },
    });
